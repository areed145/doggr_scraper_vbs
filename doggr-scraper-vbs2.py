...
##########################  FUNCTIONAL CODE BELOW HERE  #############################
    ...
    def processSelections(self):
        ...
        def vbscript(sapi):
            Popen('sys\\fetch_prodinj2.vbs '+str(sapi)+' &&exit', shell=True)
            return sapi

        def processExcel(sapi):
            scompiler = {}
            sbuild = {}
            sheader = {}
            scompile = pd.DataFrame()
            sbuild['prod'] = pd.ExcelFile('wells/'+str(sapi)+'_Production.xlsx')\
                    .parse('Well Production', header=None)
            sbuild['inj'] = pd.ExcelFile('wells/'+str(sapi)+'_Injection.xlsx')\
                    .parse('Well Injection', header=None)
            for sbuild['idx_hdr'], sbuild['data'] in enumerate(sbuild['prod'].iloc[0,:15]):
                sheader[sbuild['data']] = sbuild['prod'].iloc[1][sbuild['idx_hdr']]
            sbuild['prod_dates'] = pd.DataFrame(data=sbuild['prod'].iloc[4:,1].unique())
            sbuild['inj_dates'] = pd.DataFrame(data=sbuild['inj'].iloc[4:,1].unique())
            sbuild['dates'] = pd.DataFrame(data=sbuild['prod_dates'].append(sbuild['inj_dates'])[0].unique())
            sbuild['dates'] = sbuild['dates'][~sbuild['dates'][0].str.contains('Total')]
            sbuild['prod_pool'] = pd.DataFrame(data=sbuild['prod'].iloc[4:,15].unique())
            sbuild['inj_pool'] = pd.DataFrame(data=sbuild['inj'].iloc[4:,11].unique())
            sbuild['pools'] = pd.DataFrame(data=sbuild['prod_pool'].append(sbuild['inj_pool'])[0].unique())
            sbuild['pools'] = sbuild['pools'][pd.notnull(sbuild['pools'][0])]
            sbuild['prodinj'] = pd.DataFrame(data=sbuild['dates'][0], columns=['Month'])
            for sbuild['col'] in ['Field Name',
                        'Latitude',
                        'Lease Name',
                        'Longitude',
                        'Operator Name',
                        ...
                        'Township',
                        'Well #']:
                sbuild['prodinj'][sbuild['col']] = sheader[sbuild['col']]
            sbuild['prodinj']['API10'] = '04'+str(sheader['API #'])
            sbuild['prodinj'] = sbuild['prodinj'].rename(columns={'Well #':'Well'})
            sbuild['prod_used'] = False
            sbuild['inj_used'] = False
            if len(sbuild['prod_dates']) > 0:
                sbuild['prod_data'] = sbuild['prod'].iloc[4:,:17]
                sbuild['prod_data'].columns = sbuild['prod'].iloc[3,:17]
                sbuild['prod_data'] = sbuild['prod_data'].rename(columns={'Well Type':'Well Type Prod'})
                sbuild['prod_data'] = sbuild['prod_data'].drop(['Status', 'API Number', 'Gravity of Oil', 'Casing Pressure', 'Tubing Pressure', 'BTU', 'Method of Operation', 'Water Disposition', 'PWT Status', 'Reported Date'], axis=1)
                sbuild['prodinj'] = sbuild['prodinj'].merge(sbuild['prod_data'], how='left', left_on=['Month'], right_on=['Production Date'])
                sbuild['prodinj'] = sbuild['prodinj'].drop(['Production Date'], axis=1)
                sbuild['prod_used'] = True
            if len(sbuild['inj_dates']) > 0:
                sbuild['inj_data'] = sbuild['inj'].iloc[4:,:13]
                sbuild['inj_data'].columns = sbuild['inj'].iloc[3,:13]
                sbuild['inj_data'] = sbuild['inj_data'].rename(columns={'Well Type':'Well Type Inj'})
                sbuild['inj_data'] = sbuild['inj_data'].drop(['Status', 'API Number', 'Surface Injection Pressure', 'Source of Water', 'Kind of Water', 'PWT Status', 'Reported Date'], axis=1)
                if sbuild['prod_used'] == True:
                    sbuild['prodinj'] = sbuild['prodinj'].merge(sbuild['inj_data'], how='left', left_on=['Month','Pool Code'], right_on=['Injection Date','Pool Code'])
                else:
                    sbuild['prodinj'] = sbuild['prodinj'].merge(sbuild['inj_data'], how='left', left_on=['Month'], right_on=['Injection Date'])
                sbuild['prodinj'] = sbuild['prodinj'].drop(['Injection Date'], axis=1)
                sbuild['inj_used'] = True
            for sbuild['product'] in ['Days Well Injected', 'Days Well Produced', 'Gas Produced (Mcf)', 'Gas or Air Injected (Mcf)', 'Oil Produced (bbl)', 'Water Produced (bbl)', 'Water or Steam Injected (bbl)']:
                try:
                    sbuild['prodinj'][sbuild['product']] = sbuild['prodinj'][sbuild['product']].fillna(0)
                except:
                    pass
            sbuild['prodinj']['Month'] = pd.to_datetime(sbuild['prodinj']['Month'], format='%b-%Y')
            sbuild['prodinj']['Spud'] = pd.to_datetime(sbuild['prodinj']['Month'].min(), format='%b-%Y')
            if sbuild['prod_used'] == True:
                if sbuild['inj_used'] == True:
                    sbuild['prodinj'] = sbuild['prodinj'][(sbuild['prodinj']['Days Well Injected'] > 0) | (sbuild['prodinj']['Days Well Produced'] > 0)]
                else:
                    sbuild['prodinj'] = sbuild['prodinj'][sbuild['prodinj']['Days Well Produced'] > 0]
            else:
                if sbuild['inj_used'] == True:
                    sbuild['prodinj'] = sbuild['prodinj'][sbuild['prodinj']['Days Well Injected'] > 0]
            sbuild['prodinj'] = sbuild['prodinj'].drop_duplicates()
            scompile = scompile.append(sbuild['prodinj'])
            try:
                scompile['Cyclic Injected (bbl)'] = scompile['Water or Steam Injected (bbl)']
            except:
                pass
            try:
                scompile['Continous Injected (bbl)'] = scompile['Water or Steam Injected (bbl)']
            except:
                pass
            try:
                scompile['Water Injected (bbl)'] = scompile['Water or Steam Injected (bbl)']
            except:
                pass
            try:
                scompile.ix[scompile['Well Type Inj'] != 'SC', 'Cyclic Injected (bbl)'] = 0
            except:
                pass
            try:
                scompile.ix[scompile['Well Type Inj'] != 'SF', 'Continous Injected (bbl)'] = 0
            except:
                pass
            try:
                scompile.ix[scompile['Well Type Inj'] != 'WD', 'Water Injected (bbl)'] = 0
            except:
                pass
            try:
                scompile['Total Steam Injected (bbl)'] = scompile['Cyclic Injected (bbl)'] + scompile['Continous Injected (bbl)']
            except:
                pass
            try:
                scompile = scompile.drop(['Water or Steam Injected (bbl)'], axis=1)
            except:
                pass
            try:
                scompile = scompile.drop(['Gas or Air Injected (Mcf)'], axis=1)
            except:
                pass
    
            for scompiler['product'] in ['Days Well Injected', 'Days Well Produced', 'Gas Produced (Mcf)', 'Oil Produced (bbl)', 'Water Produced (bbl)', 'Water Injected (bbl)', 'Continous Injected (bbl)', 'Cyclic Injected (bbl)', 'Total Steam Injected (bbl)']:
                try:
                    scompile[scompiler['product']] = scompile[scompiler['product']].fillna(0)
                except:
                    pass
            scompile.to_pickle('wells/doggrScraper_'+str(sapi)+'.pk1')
            os.remove('wells/'+str(sapi)+'_Production.xlsx')
            os.remove('wells/'+str(sapi)+'_Injection.xlsx')
            
        def processExcelWrapper(sapi):
            tries=1
            while tries < 20:
                try:                    
                    processExcel(str(sapi))
                    break
                except:
                    tries=tries+1
                    Popen('sys\\fetch_prodinj2.vbs '+str(sapi)+' &&exit', shell=True)
        ...
        for i, sapi in enumerate(pool.map(vbscript, commands)):
            ...
            print(str(round(100*(i+1)/len(commands),1))+'% complete: '+str(sapi)+', '+self.timeFcst+' min(s) left')
        pool.close()
        pool.terminate()
        pool.join()
        ...
        maxprocesses=25
        pool = Pool(maxprocesses)
        for i, (tries, sapi) in enumerate(pool.map(processExcelWrapper, commands)):
            self.timeDiff = ((dt.datetime.now() - self.starttime_process).total_seconds())/60
            self.timeFcst = format(self.timeDiff * ((1/((i+1)/len(commands))) - 1), ".1f")
            print(str(round(100*(i+1)/len(commands),1))+'% complete: '+str(sapi)+' in '+str(tries)+' tries, '+self.timeFcst+' min(s) left')
        pool.close()
        pool.terminate()
        pool.join()
    ...
    def compile_wells(self):
        ...
        filenames = glob.glob('wells/doggrScraper_*.pk1')
